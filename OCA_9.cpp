...
public:
    ...
    void Processing(int dot) // Функция обхода
    {
        Visited_points[dot] = true;
        answer += to_string(dot + start_vertex) + " "; // Заполнили строчку
        for (int i = 0; i < SizeMatrix; i++)
        {
            if ((Visited_points[i] == false) && (matrix[dot][i]))
            {
                Processing(i);
            }
        }
...
int main() {
    ...
    while (command_char != 3)
    {
        ...
        {
            test.Processing(0);
            cout << " Обход до конечной: ";
            cout << test.Show_shortest_way();
